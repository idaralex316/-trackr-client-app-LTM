...
namespace TestTestServer.Controllers
{
    ...
    public class LogInController : Controller
    {
        ...
        private async Task<ActionResult<Login>> GetAd(LoginCheck login)
        {
            ...
            {
                // Hỗ trợ đăng nhập bằng email hoặc account
                var sql = "SELECT AdID, AdName, AdAccount, AdPassword, AdEmail FROM Admins Where (AdAccount = '" + login.Account.ToString() + "' OR AdEmail = '" + login.Account.ToString() + "') and AdPassword = '" + login.Password.ToString() + "'";
                connection.Open();
        ...
        private async Task<ActionResult<Login>> GetCus(LoginCheck login)
        {
            ...
            {
                // Hỗ trợ đăng nhập bằng email hoặc account
                var sql = "SELECT CusID, CusName, CusAccount, CusPassword, CusEmail FROM Customer Where (CusAccount = '" + login.Account.ToString() + "' OR CusEmail = '" + login.Account.ToString() + "') and CusPassword = '" + login.Password.ToString() + "'";
                connection.Open();
                using SqlCommand command = new SqlCommand(sql, connection);
                using SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
        ...
        private async Task<ActionResult<Login>> GetDeli(LoginCheck login)
        {
            ...
            {
                // Hỗ trợ đăng nhập bằng email hoặc account
                var sql = "SELECT ManID, ManName, ManAccount, ManPassword, ManEmail FROM DeliveryMan Where (ManAccount = '" + login.Account.ToString() + "' OR ManEmail = '" + login.Account.ToString() + "') and ManPassword = '" + login.Password.ToString() + "'";
                connection.Open();
                using SqlCommand command = new SqlCommand(sql, connection);
                using SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
